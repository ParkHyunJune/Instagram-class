...
def post_list(request):
    ...
    # Paginator 객체 생성 한페이지당 3개씩
    paginator = Paginator(all_posts, 2)

    # Get parameter에서 'page'값을 page_num 변수에 할당
    page_num = request.GET.get('page')

    # Paginator 객체에서 page 메서드로 page_num 변수를 인수로 전달하여 호출
    try:
        posts = paginator.page(page_num)
    # 만약 Page_num 변수가 int 형으로 변환이 불가능하면
    except PageNotAnInteger:
        # 1페이지에 해당하는 Post object list 를 posts 에 할당
        posts = paginator.page(1)
    except EmptyPage:
        # 마지막 페이지에 해당하는 Post object list 를 posts 에 할당
